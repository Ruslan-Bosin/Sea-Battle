...
function CreateFieldPage() {
  ...
  const [guideText, setGuideText] = useState("* Игра строится по принципу *морского боя*, за совершённые покупки на ваш аккаунт начисляются «выстрелы».\n* На виртуальном поле находятся клетки: пустые и содержащие призы. Ваша задача выбрать клетку в которой находится приз.\n\t1. Призы могут стоять в соседних клетках.\n\t2. Дважды «стрелять» одну и ту же клетку запрещено.\n\t3. На поле находитесь не вы одни, поэтому поторопитесь, **призы ограничены!**")
  const handleOk = () => {
    // guideText
